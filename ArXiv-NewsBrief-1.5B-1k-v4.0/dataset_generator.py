"""
=================================================================
📰 STEP 0.5 V4 - 뉴스 브리핑용 논문 요약 (환각 방지 강화)
=================================================================
...
✅ 규격: 1-2문장, 최대 45단어
✅ 데이터량: 3개 (테스트) → 1000개로 변경 가능

📊 V3 대비 개선:
- 환각 방지 프롬프트 강화
- 일반인 친화적 표현
- 전처리 강화 (긴 초록 대응)
- GROUP_ID=0으로 1000개 통합 생성 가능
...
GROUP_ID = 0  # 🔧 0: 전체 1000개, 1/2/3/4: 각 250개씩
...
GROUP_CONFIGS = {
    0: {  # 전체 생성 모드 (1000개)
        "START_INDEX": 2000,
        "NEW_SAMPLES": 3 if TEST_MODE else 1000,
        "MAX_INDEX": 2002 if TEST_MODE else 2999,
        "OUTPUT_FILE": "v4_training_data_test.csv" if TEST_MODE else "v4_training_data_all.csv",
        "PROGRESS_FILE": "v4_progress_test.json" if TEST_MODE else "v4_progress_all.json"
    },
    1: {  # 그룹 1 (250개)
        "START_INDEX": 2000,
        "NEW_SAMPLES": 3 if TEST_MODE else 250,
        "MAX_INDEX": 2002 if TEST_MODE else 2249,
        "OUTPUT_FILE": "v4_training_data_group1_test.csv" if TEST_MODE else "v4_training_data_group1.csv",
        "PROGRESS_FILE": "v4_progress_group1_test.json" if TEST_MODE else "v4_progress_group1.json"
    },
    2: {  # 그룹 2 (250개)
        "START_INDEX": 2250,
        "NEW_SAMPLES": 3 if TEST_MODE else 250,
        "MAX_INDEX": 2252 if TEST_MODE else 2499,
        "OUTPUT_FILE": "v4_training_data_group2_test.csv" if TEST_MODE else "v4_training_data_group2.csv",
        "PROGRESS_FILE": "v4_progress_group2_test.json" if TEST_MODE else "v4_progress_group2.json"
    },
    3: {  # 그룹 3 (250개)
        "START_INDEX": 2500,
        "NEW_SAMPLES": 3 if TEST_MODE else 250,
        "MAX_INDEX": 2502 if TEST_MODE else 2749,
        "OUTPUT_FILE": "v4_training_data_group3_test.csv" if TEST_MODE else "v4_training_data_group3.csv",
        "PROGRESS_FILE": "v4_progress_group3_test.json" if TEST_MODE else "v4_progress_group3.json"
    },
    4: {  # 그룹 4 (250개)
        "START_INDEX": 2750,
        "NEW_SAMPLES": 3 if TEST_MODE else 250,
        "MAX_INDEX": 2752 if TEST_MODE else 2999,
        "OUTPUT_FILE": "v4_training_data_group4_test.csv" if TEST_MODE else "v4_training_data_group4.csv",
        "PROGRESS_FILE": "v4_progress_group4_test.json" if TEST_MODE else "v4_progress_group4.json"
    }
}
...
USER_PROMPT_V4 = """Summarize the following text in simple, clear English that anyone can understand. Use no more than two complete sentences.
...
print("📊 V4 설정")
print(f"{'='*60}")
print(f"🎮 모드: {'테스트 (3개)' if TEST_MODE else '프로덕션 (1000개)'}")
if GROUP_ID == 0:
